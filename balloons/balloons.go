package main

import (
	"experiments/noise"
	"fmt"
	"github.com/veandco/go-sdl2/sdl"
	"image/png"
	"os"
	"time"
)

const (
	windowWidth = 800
	windowHeight = 600
)

type texture struct {
	position
	pixels []byte
	w, h, pitch int
	scale float32
}

type rgba struct {
	r, g, b byte
}

type position struct {
	x, y float32
}

func clear(pixels []byte)  {
	for i := range pixels {
		pixels[i] = 0
	}
}

func setPixel(x, y int, c rgba, pixels []byte) {
	index := (y* windowWidth + x ) * 4

	if index <= len(pixels)-4 && index >= 0 {
		pixels[index] = c.r
		pixels[index + 1] = c.g
		pixels[index + 2] = c.b
	}
}

func (t *texture) drawScaled(scaleX, scaleY float32, pixels []byte) {
	newWidth := int(float32(t.w) * scaleX)
	newHeight := int(float32(t.h) * scaleY)
	tW4 := t.w * 4
	for y := 0; y < newHeight; y++ {
		fy := float32(y) / float32(newHeight) * float32(t.h - 1)
		fyi := int(fy)
		screenY := int(fy * scaleY) + int(t.y)
		screenIndex := screenY * windowWidth * 4 + int(t.x) * 4
		for x := 0; x < newWidth; x++ {
			fx := float32(x) / float32(newWidth) * float32(t.w - 1)
			screenX := int(fx * scaleX) + int(t.x)
			if screenX >= 0 && screenX < windowWidth && screenY >= 0 && screenY < windowHeight {
				fxi4 := int(fx) * 4
				pixels[screenIndex] = t.pixels[fyi * tW4 + fxi4]
				screenIndex++
				pixels[screenIndex] = t.pixels[fyi * tW4 + fxi4 + 1]
				screenIndex++
				pixels[screenIndex] = t.pixels[fyi * tW4 + fxi4 + 2]
				screenIndex++
				screenIndex++ // skip alpha
			}

			//screenIndex := screenY * windowWidth * 4 + int(t.x) * 4
		}
	}
}

func flerp1(a, b, pct float32) float32 {
	return a + (b - a) * pct
}

func blerp(c00, c10, c01, c11, tx, ty float32) float32 {
	return flerp1(flerp1(c00, c10, tx), flerp1(c01, c11, tx), ty)
}

func (t *texture) drawBilinearScaled(scaleX, scaleY float32, pixels []byte) {
	newWidth := int(float32(t.w) * scaleX)
	newHeight := int(float32(t.h) * scaleY)
	tW4 := t.w * 4
	for y := 0; y < newHeight; y++ {
		fy := float32(y) / float32(newHeight) * float32(t.h - 1)
		fyi := int(fy)
		screenY := int(fy * scaleY) + int(t.y)
		screenIndex := screenY * windowWidth * 4 + int(t.x) * 4
		ty := fy - float32(fyi)
		for x := 0; x < newWidth; x++ {
			fx := float32(x) / float32(newWidth) * float32(t.w - 1)
			screenX := int(fx * scaleX) + int(t.x)
			if screenX >= 0 && screenX < windowWidth && screenY >= 0 && screenY < windowHeight {
				fxi := int(fx)

				var (
					c00i = fyi * tW4 + fxi * 4
					c10i = fyi * tW4 + (fxi + 1) * 4
					c01i = (fyi + 1) * tW4 + fxi * 4
					c11i = (fyi + 1) * tW4 + (fxi + 1) * 4

					tx = fx - float32(fxi)
				)

				for i := 0; i < 4; i++ {
					var (
						c00 = float32(t.pixels[c00i + i])
						c10 = float32(t.pixels[c10i + i])
						c01 = float32(t.pixels[c01i + i])
						c11 = float32(t.pixels[c11i + i])
					)
					pixels[screenIndex] = byte(blerp(c00, c10, c01, c11, tx, ty))
					screenIndex++
				}
			}
		}
	}
}
func (t *texture) draw(pixels []byte) {
	for y := 0; y < t.h; y++ {
		for x := 0; x < t.w ; x++ {
			screenY := y + int(t.y)
			screenX := x + int(t.x)
			if screenX >= 0 && screenX <= windowWidth && screenY >= 0 && screenY <= windowHeight {
				tIndex := y * t.pitch + x * 4
				screenIndex := screenY * windowWidth * 4 + screenX * 4

				pixels[screenIndex] = t.pixels[tIndex]
				pixels[screenIndex + 1] = t.pixels[tIndex + 1]
				pixels[screenIndex + 2] = t.pixels[tIndex + 2]
				pixels[screenIndex + 3] = t.pixels[tIndex + 3]
			}
		}
	}
}

func (t *texture) drawAlpha( pixels []byte)  {
	for y := 0; y < t.h; y++ {
		screenY := y + int(t.y)
		for x := 0; x < t.w ; x++ {
			screenX := x + int(t.x)
			if screenX >= 0 && screenX <= windowWidth && screenY >= 0 && screenY <= windowHeight {
				tIndex := y * t.pitch + x * 4
				screenIndex := screenY * windowWidth * 4 + screenX * 4
				var (
					srcR = int(t.pixels[tIndex])
					srcG = int(t.pixels[tIndex + 1])
					srcB = int(t.pixels[tIndex + 2])
					srcA = int(t.pixels[tIndex + 3])
				)

				var (
					dstR = int(pixels[screenIndex])
					dstG = int(pixels[screenIndex + 1])
					dstB = int(pixels[screenIndex + 2])
				)
				var (
					rstR = (srcR * 255 + dstR * (255 - srcA)) / 255
					rstG = (srcG * 255 + dstG * (255 - srcA)) / 255
					rstB = (srcB * 255 + dstB * (255 - srcA)) / 255
				)

				pixels[screenIndex] = byte(rstR)
				pixels[screenIndex + 1] = byte(rstG)
				pixels[screenIndex + 2] = byte(rstB)
			}
		}
	}
}

func loadBalloons() []texture {

	balloonNames := []string {
		"C:/Users/xpoc_/go/src/experiments/balloons/balloon_red.png",
		"C:/Users/xpoc_/go/src/experiments/balloons/balloon_green.png",
		"C:/Users/xpoc_/go/src/experiments/balloons/balloon_blue.png"}

	balloonTextures := make([]texture, len(balloonNames))

	for i, name := range balloonNames {

		file, err := os.Open(name)
		if err != nil {
			panic(err)
		}
		defer file.Close()

		img, err := png.Decode(file)
		if err != nil {
			panic(err)
		}
		w := img.Bounds().Max.X
		h := img.Bounds().Max.Y

		balloonPixels := make([]byte, w * h * 4)
		bIndex := 0
		for y := 0; y < h; y++ {
			for x := 0; x < w; x++ {
				r, g, b, a := img.At(x, y).RGBA()
				balloonPixels[bIndex] = byte(r / 256)
				bIndex++
				balloonPixels[bIndex] = byte(g / 256)
				bIndex++
				balloonPixels[bIndex] = byte(b / 256)
				bIndex++
				balloonPixels[bIndex] = byte(a / 256)
				bIndex++
			}
		}
		balloonTextures[i] = texture{position{float32(i * 60),float32(i * 60)},balloonPixels,w, h, w * 4, float32(i) + 1}
	}
	return balloonTextures
}

func flerp(b1, b2 byte, pct float32) byte {
	return byte(float32(b1) + pct * (float32(b2) - float32(b1)))
}

func colorLerp(c1, c2 rgba, pct float32) rgba {
	return rgba{flerp(c1.r, c2.r, pct), flerp(c1.g, c2.g, pct), flerp(c1.b, c2.b, pct)}
}

func getGradient(c1, c2 rgba) []rgba {
	result := make([]rgba, 256)
	for i := range result {
		pct := float32(i) / float32(255)
		result[i] = colorLerp(c1, c2, pct)
	}
	return result
}

func getDualGradient(c1, c2 , c3, c4 rgba) []rgba {
	result := make([]rgba, 256)
	for i := range result {
		pct := float32(i) / float32(255)
		if pct < .5 {
			result[i] = colorLerp(c1, c2, pct * 2)
		} else {
			result[i] = colorLerp(c3, c4, pct * 1.5 - .5)
		}
	}
	return result
}

func clamp(min, max, v int) int {
	if v < min {
		v = min
	} else if v > max {
		v = max
	}
	return v
}

func rescaleAndDraw(noise []float32, min, max float32, gradient []rgba, w, h int) []byte {
	pixels := make([]byte, w * h * 4)
	scale := 255.0 / (max - min)
	offset := min * scale
	for i := range noise {
		noise[i] = noise[i]*scale - offset
		//b := byte(noise[i])
		c := gradient[clamp(0,255, int(noise[i]))]
		p := i * 4
		pixels[p] = c.r
		pixels[p + 1] = c.g
		pixels[p + 2] = c.b
	}
	return pixels
}

func main()  {
	if err := sdl.Init(sdl.INIT_EVERYTHING); err != nil {
		panic(err)
	}
	defer sdl.Quit()

	windows, err := sdl.CreateWindow("Testing", sdl.WINDOWPOS_UNDEFINED, sdl.WINDOWPOS_UNDEFINED, windowWidth, windowHeight,
		sdl.WINDOW_SHOWN)
	if err != nil {
		panic(err)
		return
	}
	defer  windows.Destroy()

	renderer, err := sdl.CreateRenderer(windows, -1, sdl.RENDERER_ACCELERATED)
	if err != nil {
		panic(err)
		return
	}
	defer renderer.Destroy()
	tex, err := renderer.CreateTexture(sdl.PIXELFORMAT_ABGR8888, sdl.TEXTUREACCESS_STREAMING, windowWidth, windowHeight)
	if err != nil {
		panic(err)
		return
	}
	defer tex.Destroy()

	cloudNoise, min, max := noise.MakeNoise(noise.FBM, .009, .5,3,3, windowWidth, windowHeight)
	cloudGradient := getGradient(rgba{0,0,255}, rgba{255,255,255})
	cloudPixels := rescaleAndDraw(cloudNoise, min, max, cloudGradient, windowWidth, windowHeight)
	cloudTexture := texture{position{0,0}, cloudPixels, windowWidth, windowHeight, windowWidth * 4, 1}

	pixels := make([]byte, (windowWidth * windowHeight) * 4)
	balloonTextures := loadBalloons()
	var dir = 1
	for {
		frameStart := time.Now()
		for event := sdl.PollEvent(); event != nil; event = sdl.PollEvent() {
			switch event.(type) {
			case *sdl.QuitEvent:
				return
			}
		}
		cloudTexture.draw(pixels)
		for _, t := range balloonTextures {
			t.drawBilinearScaled(t.scale, t.scale, pixels)
		}

		balloonTextures[1].x += float32(1 * dir)
		if balloonTextures[1].x > 400 || balloonTextures[1].x < 0 {
			dir = dir * -1
		}
		if err = tex.Update(nil, pixels, windowWidth * 4); err != nil {
			panic(err)
		}
		if err = renderer.Copy(tex, nil,nil); err != nil {
			panic(err)
		}
		renderer.Present()
		elapsedTime := float32(time.Since(frameStart).Seconds() * 1000)
		if elapsedTime < 5 {
			sdl.Delay(5 - uint32(elapsedTime))
			elapsedTime = float32(time.Since(frameStart).Seconds())

		}
		fmt.Println(elapsedTime)
		sdl.Delay(16)
	}
}
