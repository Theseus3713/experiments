...
// MakeNoise generates a 2d block og noise
func MakeNoise(noiseType NoiseType, frequency, lacunarity, gain float32, octaves, w, h int) ([]float32, float32, float32) {
	//startTime := time.Now()
	...
	numRutines := runtime.NumCPU()
	batchxSize := len(noise) / numRutines
	minMaxChan := make(chan float32, numRutines * 2)

	for i := 0; i < numRutines; i++ {
		wg.Add(1)
		go func(i int, wg *sync.WaitGroup) {
			defer wg.Done()
			var innerMax float32
			var innerMin float32
			start := i * batchxSize
			end := start + batchxSize - 1
			for j := start; j < end ; j++ {
				x := j % w
				y := (j - x) / w
				...
				switch noiseType {
					...
					default:
						panic(`noise type is not exists`)
				}
				if noise[j] < innerMin {
					innerMin = noise[j]
