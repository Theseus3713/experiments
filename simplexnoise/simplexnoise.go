...
func makeNoise(pixels []byte, frequency, lacunarity, gain float32, octaves, w, h int) {
	startTime := time.Now()
	noise := make([]float32, windowWidth*windowHeight)
	...
	fmt.Println("Lead time function makeNoise:", elapsedTime)
...
func main()  {
	...
	var frequency float32 = .01
	var gain float32 = .2
	var lacunarity float32 = 3.0
	var octaves = 3
	makeNoise(pixels, frequency, lacunarity, gain, octaves, windowWidth, windowHeight)
